...
function Header() {
  ...
  return (
    <header className={`z-50 fixed top-0 left-0 right-0 transition-colors duration-300 ${
      isMenuOpen ? 'bg-white/95 dark:bg-black/95 backdrop-blur-lg' :isScrolled ? 'bg-white/80 dark:bg-black/40 backdrop-blur-sm' : 'bg-transparent'
    }`}>
      <div className="flex justify-between items-center w-full p-4 md:pt-5 md:px-8">
        ...
        >
          <motion.div
            ...
            className="space-y-2"
          >
            <span className={`block w-8 h-0.5 bg-current transform transition-all duration-300 ${isMenuOpen ? 'rotate-45 translate-y-2.5' : ''}`} />
            <span className={`block w-8 h-0.5 bg-current transition-all duration-300 ${isMenuOpen ? 'opacity-0' : ''}`} />
            <span className={`block w-8 h-0.5 bg-current transform transition-all duration-300 ${isMenuOpen ? '-rotate-45 -translate-y-2.5' : ''}`} />
          </motion.div>
        </button>
      </div>

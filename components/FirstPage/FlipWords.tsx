...
function FlipWordsCp({ mode = "dev" }: { mode?: "dev" | "ml" }) {
  const words =
    mode === "ml"
      ? ["datasets", "features", "models", "evaluation", "inference"]
      : ["detail", "quality", "accuracy", "craftsmanship", "thoroughness"];

  const title = mode === "ml" ? "Welcome to ML" : "Welcome";
  const action = mode === "ml" ? "Optimize" : "Emphasize";
    ...
    <div className="h-full flex flex-col justify-center items-center z-20 px-4">
      <div className="relative z-20 text-center mt-20">
        ...
        <div className="text-base sm:text-lg md:text-xl lg:text-2xl font-normal text-secondary dark:text-foreground-muted">
          <motion.span layout>{action}</motion.span>
          <FlipWords
            words={words}
